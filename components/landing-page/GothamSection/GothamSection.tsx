...
export default function GothamSection() {
  ...
  const [userEmail, setUserEmail] = useState("");
  const [userPhone, setUserPhone] = useState("");
  const [paymentCompleted, setPaymentCompleted] = useState(true); // ✅ Always true to skip payment
  ...
  // 🧠 Direct verification logic — payment no longer required
  const handleVerify = useCallback(async () => {
    ...
    try {
      ...
      // Default paymentDetails placeholder (not used now)
      const finalResult: VerificationResult = {
        ...data,
        paymentDetails: {
          localAmount: 0,
        ...
        <div className="mx-auto max-w-2xl px-6">
          <Card className="shadow-lg border border-dashed border-customTeal bg-foreground/10">
            <CardContent className="p-8 space-y-10">
              ...
              {/* ✅ Direct Verify Button (Payment Removed) */}
